...
_DEBOUNCE_MS = 100   # ms to wait after last settings change before redrawing

from config import (
    DARK_BG, PANEL_BG, SUBTEXT, BORDER, ACCENT,
    SCALE_CONFIG, DATASET_MAP,
)
...
class _PlotArea(QWidget):
    """
    Wrapper that fills its parent normally, but when constrained=True it centres
    the inner widget at a 1:1 aspect ratio (square) using manual geometry.
    """
    ...
    def set_constrained(self, c: bool) -> None:
        self._constrained = c
        self._relayout()
    ...
    def _relayout(self) -> None:
        ...
        if self._constrained:
            side = min(w, h)
            x = (w - side) // 2
            self._inner.setGeometry(x, 0, side, h)
        else:
...
class GraphPanel(QWidget):
    ...
    def __init__(self, letter: str, parent=None):
        ...
        self._plot_area = _PlotArea(self.plot_widget)
        layout.addWidget(self.ctrl)
        layout.addWidget(self._plot_area, stretch=1)
        ...
        self.ctrl.settings_changed.connect(self._redraw_timer.start)
        self.ctrl.save_btn.clicked.connect(
            lambda: self.ctrl.do_save(self.plot_widget.canvas,
                                      self.plot_widget._last_filtered))
        self.redraw()
    ...
    def set_fullscreen(self, fs: bool) -> None:
        self._fullscreen = fs
        self.ctrl.setVisible(not fs)
        self._plot_area.set_constrained(fs)
...
class DualGraphPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._plots_splitter = QSplitter(Qt.Vertical)
        self._plots_splitter.addWidget(self.plot_a)
        self._plots_splitter.addWidget(self.plot_b)
        self._plots_splitter.setSizes([500, 500])
        ...
        self._timer_a = QTimer()
        self._timer_a.setSingleShot(True)
        self._timer_a.setInterval(_DEBOUNCE_MS)
        self._timer_a.timeout.connect(lambda: self._redraw('A'))

        self._timer_b = QTimer()
        self._timer_b.setSingleShot(True)
        self._timer_b.setInterval(_DEBOUNCE_MS)
        self._timer_b.timeout.connect(lambda: self._redraw('B'))
        ...
        self.ctrl_a.save_btn.clicked.connect(
            lambda: self.ctrl_a.do_save(self.plot_a.canvas, self.plot_a._last_filtered))
        self.ctrl_b.save_btn.clicked.connect(
            lambda: self.ctrl_b.do_save(self.plot_b.canvas, self.plot_b._last_filtered))
    ...
    def set_fullscreen(self, fs: bool) -> None:
        self._ctrl_container.setVisible(not fs)
        self._plots_splitter.setOrientation(Qt.Horizontal if fs else Qt.Vertical)
