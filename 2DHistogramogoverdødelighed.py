import sys, os, copy, json, pickle, numpy as np
...
CACHE_FILE       = 'data_cache.pkl'    # pickle cache — avoids slow dataloader on repeat runs
...
class PlotCanvas(FigureCanvas):
    ...
    def draw_histogram(self, data, kassationsårsag, bins, min_dage, max_dage,
        ...
        if ax is not None:
            ...
            if ax_hist is None: ax.set_xlabel(xlabel, color=SUBTEXT)
        ...
        if ax_hist is not None:
            ...
            ax_hist.plot(x_hist_vals, baseline + 4*std, color='#f38ba8', lw=1.4, ls='-.', alpha=0.85, label='4 z-score')
            ax_hist.plot(x_hist_vals, baseline + 2*std, color='#fab387', lw=1.4, ls='--', alpha=0.85, label='2 z-score')
            ax_hist.plot(x_hist_vals, baseline,          color=INFO,      lw=2.2, label='Forventet')
            ax_hist.plot(x_hist_vals, counts,            color=TEXT,      lw=1.8, alpha=0.92, label='Registreret')

            over2 = counts > (baseline + 2*std)
            if over2.any():
                ax_hist.fill_between(x_hist_vals, baseline + 2*std, counts,
                                     where=over2, alpha=0.3, color='#f9e2af', label='Over tærskel')
            ...
            ax_hist.legend(lines_1 + lines_2, labels_1 + labels_2,
                           ...
                           facecolor=PANEL_BG, edgecolor=BORDER, labelcolor=TEXT, ncol=2)
        ...
        if show_regression and can_regress and ax is not None:
            self.fig.text(0.5, 0.01, stats_text,
...
class MainWindow(QMainWindow):
    def __init__(self, on_refresh=None):
        ...
        if on_refresh:
            ...
            refresh_btn.setToolTip("Genindlæser data fra dataloader og opdaterer cachen")
            refresh_btn.setStyleSheet(
                f"QPushButton {{ background:#13131f; color:{INFO}; border:1px solid {BORDER}; "
...
def main():
    ...
    def _load_from_dataloader():
        """Call dataloader and return a plain dict of DataFrames."""
    ...
    def _apply_frames(frames):
        """Push a frames dict into global variables and DATASET_MAP."""
        ...
        samlet_df       = frames['samlet_df']
        skjorte_data    = frames['skjorte_data']
        shorts_data     = frames['shorts_data']
        bukse_data      = frames['bukse_data']
        tshirt_data     = frames['tshirt_data']
        langærmet_data  = frames['langærmet_data']
        jakke_data      = frames['jakke_data']
        fleece_data     = frames['fleece_data']
        overall_data    = frames['overall_data']
        forklæde_data   = frames['forklæde_data']
        kittel_data     = frames['kittel_data']
        busseron_data   = frames['busseron_data']
        kokkejakke_data = frames['kokkejakke_data']
        andre_data      = frames['andre_data']
        DATASET_MAP.update({
    ...
    def step1_data(force_refresh=False):
        ...
        try:
            splash.set_status("Indlæser data fra dataloader…")
            app.processEvents()
            ...
            splash.set_status("Gemmer cache…")
            app.processEvents()
            try:
    ...
    def step2_build():
        ...
        app.processEvents()
        win = MainWindow(on_refresh=lambda: _do_refresh(win))
        app._main_win = win
    ...
    def _do_refresh(win):
        """Called when the user clicks 🔄 Opdater data — shows splash and reloads."""
        splash.show()
        app.processEvents()
        # Close current window, reload data, rebuild UI
        win.close()
        step1_data(force_refresh=True)
